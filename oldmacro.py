...
class MouseClickRecorderUI:
    def __init__(self, master):
        ...
        self.recent_files_listbox = CTkListbox(master, height=50)  # Use CTkListbox instead of CTkTextbox
        self.recent_files_listbox.grid(row=7, column=0, columnspan=2, padx=(25, 0), pady=(10, 10))

        # Set up hotkeys
        kb.add_hotkey('R', self.start_recording)  
        kb.add_hotkey('S', self.stop_recording)   
        kb.add_hotkey('P', self.play_macro)       
        kb.add_hotkey('Z', self.save_macro)       
        kb.add_hotkey('L', self.load_macro)       
        kb.add_hotkey('D', self.reset_macro)      
    ...
    def _play_macro_thread(self):
        self.recorder.play_macro()
        # Use after to update the status back to "Idle" on the main thread
    ...
    def add_recent_file(self, filename):
        ...
        if len(self.recent_files) > 5:  # Change to 5 to limit to 5 recent files
...
class MouseClickRecorder:
    ...
    def on_click(self, x, y, button, pressed):
        if self.recording:
            ...
            print(f"Recorded click: {event}")  # Debug print
    ...
    def play_macro(self):
        if not self.events:
            print("No events to play.")  # Debug print
            return
        ...
        for i, event in enumerate(self.events):
            ...
            if event['type'] == 'click':
                ...
                if event['pressed']:
                    ...
                    print(f"Played click at ({x}, {y})")  # Debug print
